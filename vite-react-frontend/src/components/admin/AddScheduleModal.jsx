...
const AddScheduleModal = ({ show, onHide, onScheduleAdded, doctor }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response?.status === 409) {
                ...
                setError(backendMessage || 'Schedule Conflict: This time slot overlaps with an existing schedule. Please choose a different time range.');
            } else {
                setError(error.response?.data?.message || 'Failed to add schedule block');
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <Modal show={show} onHide={onHide}>
            <Modal.Header closeButton>
                <Modal.Title>Add Schedule Block</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                ...
                <Alert variant="info">
                    ...
                    <strong>Clinic:</strong> {doctor?.clinicName || `Clinic #${doctor?.clinicId}`}
                </Alert>
